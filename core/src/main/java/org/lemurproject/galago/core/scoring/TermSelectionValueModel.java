...
public class TermSelectionValueModel implements ExpansionModel {
  ...
  public void initialize() throws IOException {
    ...
    Node dummy = TextPartAssigner.assignPart(new Node("extents", "dummy"), new Parameters(), retrieval.getGlobalParameters());
  ...
  public Node generateExpansionQuery(List<ScoredDocument> initialResults, int fbTerms,
    ...
    for (int i = 0; i < scored.size() && expanded < fbTerms; i++) {
      ...
      Node inner = TextPartAssigner.assignPart(new Node("extents", g.term), new Parameters(), retrieval.getGlobalParameters());
