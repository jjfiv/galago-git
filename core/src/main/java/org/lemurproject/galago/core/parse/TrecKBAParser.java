...
public class TrecKBAParser implements DocumentStreamParser {
    ...
    public TrecKBAParser(DocumentSplit split, InputStream stream ) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        InputStream is = new BufferedInputStream(new XZInputStream(new FileInputStream(testFile), 100*1024));
        DocumentSplit split = new DocumentSplit(testFile.getAbsolutePath(), "", false, new byte[0], new byte[0], 0, 0);

        TrecKBAParser parser = new TrecKBAParser(split,is);
