 ...
 * arrays of bytes. 
 *
 * We assume the buffer will have to work with variable sizes, so we start with small
 * allocation (2 bytes), and harmonically increase by a factor of 2 up to sub-buffers of
 * 32K. Starting w/ 2 provides the guarantee that if we write anything the the buffer at all,
 * our utilization will never be below 50%.
 *
 * Adding in copies of another buffer will not disrupt the harmonic progression, because,
 * there's no reason to believe we need to allocate larger sub-buffers. It will, however,
 * close out the current open sub-buffer if there is one and open a new one of the same size.
 *
 * (irmarc): I'm also going to assume right now that we'll eventually get to a buffer size too
 *           big for memory. Therefore, we spill to disk every 32MB. Once the overflow file is open,
 *           we stream to it every 32MB, therefore we never keep more than 32MB in memory.
 *           When called to write, we close the file, let Utility.copyFileToStream do the work.
 *           clear() will delete the temp file, otherwise it's cleared when galago cleans up.
 *
 * @author sjh
 * @author irmarc
 */
...
public class CompressedRawByteBuffer extends OutputStream {
	...
	if (position == currentBufferSize) {
	    ...
	    if (currentBufferSize < hi) currentBufferSize *= 2;
	...
	if (spillStream != null) {
      ...
      if (sizes==null) sizes = new TIntArrayList();
      else sizes.clear();
      currentBuffer = null;
